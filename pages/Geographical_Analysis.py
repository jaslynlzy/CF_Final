...
if 'df2' in st.session_state:
    ...
    with st.sidebar:
        ...
        if selected_tab == "Vouchers":
            ...
            max_household_size = max(int(cleaned_df["household_size"].max()),2)
            st.markdown("<h4 style='margin-top: 15px; margin-bottom: -40px;'>Household size</h4>", unsafe_allow_html=True)
            filter_household_size = st.slider("", 1, 1, (1, 1), key="filter_household_size_voucher")
            ...
            st.markdown("<h4 style='margin-top: -10px;'></h4>", unsafe_allow_html=True)
    ...
    if st.session_state.filter_delivery is not None:
    ...
    filtered_df_foodbanks = filtered_df[cleaned_df["assigned food bank centre"].isin(st.session_state.filter_foodbank)]
